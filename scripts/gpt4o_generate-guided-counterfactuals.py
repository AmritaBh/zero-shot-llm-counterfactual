...
def main(args):
    ...
    try:
        out_df['words_chosen'] = words_chosen
        out_df['counterfactual_text'] = cf_explanations
        out_df['noisy_cf'] = noisy_explanations
        


        out_df.to_csv(f"words-only-{MODEL}-" + test_type + "-cf-explanations.csv", index=False)
